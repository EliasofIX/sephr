...
actor InferenceWorker {
    ...
    private static let generationOptions = GenerationOptions()
        ...
        .with(maxTokens: InferenceBudget.maxOutputTokens)
    ...
    private func trimUserPrompt(
        ...
        let normalized = TextBudget.normalizeForModel(user)
        let charCeiling = InferenceBudget.estimatedUserCharCeiling
        var candidate = PromptTrimmer.pretrimForTokenization(
            normalized, maxChars: charCeiling)
        ...
        let userBudget = max(
            512,
            InferenceBudget.softPromptTokenCeiling - systemTokens)
        ...
        guard total > InferenceBudget.softPromptTokenCeiling else {
            return candidate
        }

        if let trimmed = try await PromptTrimmer.trimSuperBrowseSources(
            in: candidate,
            overBudget: { text in
            ...
            total = Int(
                try await runner.getPromptTokensSize(
                    messages: [systemMessage, userMessage],
                    addBosToken: true).intValue)
            if total <= InferenceBudget.softPromptTokenCeiling {
                return candidate
            }
        }
        ...
        while paragraphs.count > 1,
              total > InferenceBudget.softPromptTokenCeiling {
        ...
        if total > InferenceBudget.softPromptTokenCeiling {
            let charBudget = InferenceBudget.estimatedCharBudget(
                forTokenBudget: userBudget)
            return TextBudget.truncate(candidate, to: charBudget)
        }
        return candidate
    }
}
